...
export interface DropMeta {
  ...
  /** Sequence number of the version currently published. */
  version: number;
  created_at: string;
  updated_at: string;
}

/** One entry of a drop's publication history. */
export interface VersionInfo {
  seq: number;
  entrypoint: string;
  files: number;
  size: number;
  /** True for the version served at the drop's own URL. */
  current: boolean;
  /** Pins this snapshot; keeps working after later uploads. */
  url: string;
  published_at: string;
}
